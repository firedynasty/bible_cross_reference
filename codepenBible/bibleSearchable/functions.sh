...
# Read Matthew Henry Commentary: commentary genesis 1, commentary gen 5
# Use chapter 0 for preface: commentary genesis 0
commentary() {
  local book="$1"
  local chapter="${2:-1}"

  if [[ -z "$book" ]]; then
    echo "Usage: commentary <book> [chapter]"
    echo "Examples:"
    echo "  commentary genesis 1"
    echo "  commentary gen 5"
