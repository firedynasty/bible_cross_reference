...
['1', 'מים', 'Water']
  ...
  const speakText = (text, cellIndex) => {
    ...
    // Handle Hebrew voice selection
    else if (selectedLanguage === 'he-IL' && cellIndex !== 2) {
      let hebrewVoice;
      
      // 1. Try Google Hebrew voices first
      hebrewVoice = voices.find(voice => 
        voice.name.includes('Google') && voice.lang.includes('he'));
      
      // 2. Try Enhanced/Premium Hebrew voices
      if (!hebrewVoice) {
        hebrewVoice = voices.find(voice => 
          voice.lang.includes('he') && (voice.name.includes('Enhanced') || voice.name.includes('Premium')));
      }
      
      // 3. Try specific known good Hebrew voices
      if (!hebrewVoice) {
        const goodHebrewNames = [
          'Asaf',       // First priority - Male Hebrew voice (commonly available)
          'Carmit',     // Second priority - Common Israeli Hebrew voice
          'Microsoft Asaf',  // Microsoft Hebrew voices
          'Microsoft Carmit',
          'Zohar',      // Hebrew voice variant
          'Lior',       // Less commonly available
          'Hebrew',     // Generic Hebrew voice
          'Apple Hebrew',    // Apple Hebrew voices
          'Siri Hebrew'
        ];
      ...
      // 4. Try any Hebrew voice (he-IL, he, etc.)
      if (!hebrewVoice) {
      ...
      if (hebrewVoice) {
        ...
        console.log('No Hebrew voice found. Available voices:', voices.map(v => `${v.name} (${v.lang})`));
        setLastUsedVoice('No Hebrew voice available on this system');
      }
    }
