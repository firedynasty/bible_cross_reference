...
def parse_pasted(text):
    ...
    # Format A: timestamp + text on same line
    cues = []
...
def main():
    ...
    for ch in range(1, total_chapters + 1):
        anchors = fetch_chapter_anchors(book_abbrev, ch)

        if not anchors:
            print(f"  Ch {ch:3d}: [skip — no anchor text fetched]")
            continue

        secs, score = find_best_match(cues, anchors)
        ...
        if secs is not None:
            ...
            print(f"  Ch {ch:3d}: {ts:>9s}  ({secs:5d}s)  score={score:.2f}{flag}"
