...
const BibleApp = () => {
  ...
  useEffect(() => {
    ...
    const handleKeyDown = (e) => {
      ...
      // ',' / '.' — line-by-line navigation with scroll (like PHP viewer)
      else if ((e.key === ',' || e.key === '.') && !e.metaKey && !e.ctrlKey && !e.altKey) {
        // Find the active scrollable text pane
        const sel = window.getSelection();
        let lnContainer = null;

        // Only target pane 2 (right pane / KJV pane)
        if (sel && sel.rangeCount > 0) {
          let node = sel.anchorNode;
          while (node) {
            if (node === kjvContentRef.current) {
              lnContainer = node; break;
            }
            node = node.parentNode;
          }
        }
        if (!lnContainer && kjvContentRef.current) lnContainer = kjvContentRef.current;
        if (!lnContainer) return;

        const lnFullText = lnContainer.textContent;
        const lnLines = lnFullText.split('\n').filter(l => l.trim().length > 0);
        if (!lnLines.length) return;

        // Helper: char offset of selection start within container
        const getOffset = (root, targetNode, targetOff) => {
          let pos = 0;
          const walk = (n) => {
        ...
        // Find current line in the filtered list
        let lnCurLine = -1;
        const lnHasSel = sel && sel.toString().trim().length > 0;
        if (lnHasSel && sel.rangeCount > 0) {
          const range = sel.getRangeAt(0);
          const startOff = getOffset(lnContainer, range.startContainer, range.startOffset);
          // Find offset in full text for each filtered line
          let search = lnFullText;
          let cumOff = 0;
          for (let li = 0; li < lnLines.length; li++) {
            const idx = search.indexOf(lnLines[li]);
            cumOff += idx;
            if (startOff >= cumOff && startOff < cumOff + lnLines[li].length) { lnCurLine = li; break; }
            cumOff += lnLines[li].length;
            search = lnFullText.slice(cumOff);
          }
        }

        const lnTarget = e.key === ',' ? lnCurLine - 1 : lnCurLine + 1;
        if (lnTarget < 0 || lnTarget >= lnLines.length) return;
        e.preventDefault();

        // Select the target line text using DOM range
        const targetText = lnLines[lnTarget];
        const makeRange = (root, text) => {
          let found = null;
          const walk = (n) => {
            if (found) return;
            if (n.nodeType === 3) {
        ...
        const newRange = makeRange(lnContainer, targetText);
        if (!newRange) return;
        sel.removeAllRanges();
        sel.addRange(newRange);

        // Scroll to keep highlighted line centered
