...
const NavigationPlaceholder = ({ 
    ...
    if (autoScrollActive) {
      intervalId = setInterval(() => {
        ...
        if (now - lastScrollTimeRef.current >= scrollIntervalSeconds * 1000) {
          ...
          // Check if we've been at the bottom for consecutive intervals
          let atEnd = true;
          scrollContainers.forEach(container => {
            ...
            // Save previous position for comparison
            const lastScrollPos = window.lastScrollPositions[id] || 0;

            // Check if position changed since last time
            if (Math.abs(currentScrollPos - lastScrollPos) > 1) {
              atEnd = false;
            }

            // Update saved position
            window.lastScrollPositions[id] = currentScrollPos;
          });

          // Always simulate key press
          simulateZKeyPress();

          // Only play sound if we're not at the end of the page
          // This prevents sound when scrolling can't happen anymore
          if (!atEnd) {
            playSubtleBeep();
          }
  ...
  }, [autoScrollActive, simulateZKeyPress, scrollIntervalSeconds, playSubtleBeep]);
  ...
  return (
    <div className="relative">
      ...
      <div className="flex items-center bg-blue-50 px-2 py-1 rounded-md text-blue-800 text-sm">
        ...
        >
          {autoScrollActive ? 'SCROLL ON' : 'SCROLL OFF'}
        </button>
